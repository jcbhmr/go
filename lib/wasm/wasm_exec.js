		...
		constructor() {
			...
			this.importObject = {
				...
				gojs: {
					...
					"syscall/js.valueDeleteSymbol": (sp) => {
						sp >>>= 0;
					...
					"syscall/js.valueCallSymbol": (sp) => {
						sp >>>= 0;
						try {
							const v = loadValue(sp + 8);
							const m = Reflect.get(v, loadValue(sp + 16));
							const args = loadSliceOfValues(sp + 24);
							const result = Reflect.apply(m, v, args);
							sp = this._inst.exports.getsp() >>> 0; // see comment above
							storeValue(sp + 48, result);
							this.mem.setUint8(sp + 56, 1);
						} catch (err) {
							sp = this._inst.exports.getsp() >>> 0; // see comment above
							storeValue(sp + 48, err);
							this.mem.setUint8(sp + 56, 0);
						}
					},
